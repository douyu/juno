          ...
          renderItem={(item) => {
            return (
              <List.Item
                extra={
                  <Popconfirm
                    title={'请谨慎操作，删除后该第三方应用无法通过Open API访问本系统.确认删除?'}
                    onConfirm={() => {
                ...
                <List.Item.Meta
                  ...
                  description={
                    <div>
                      ...
                      <div>
                        App Secret:{' '}
                        {secretVisibleId === item.app_id
                          ? item.app_secret
                          : '*'.repeat(item.app_secret.length)}
                        <Button
                          type={'link'}
                          onClick={() =>
                            setSecretVisibleId(secretVisibleId !== item.app_id ? item.app_id : '')
                          }
                        >
                          {secretVisibleId === item.app_id ? (
                            <EyeOutlined />
