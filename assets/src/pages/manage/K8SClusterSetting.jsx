...
function ClusterForm(props) {
  ...
  const zoneCodes = {}
  const envs = []
  zoneEnvResp && Object.keys(zoneEnvResp.data).forEach(env => {
    envs.push(env)
  ...
  return <Form form={form} onFinish={onOk}>
    ...
    <Form.Item label={"Env"} name={"env"}>
      <Select mode={"tags"} loading={zoneEnvLoading}>
        {envs.map(env => <Select.Option value={env}>{env}</Select.Option>)}
      </Select>
    </Form.Item>
    ...
    <Form.Item label={"Zone Code"} name={"zone_code"}>
      <AutoComplete>
        {Object.keys(zoneCodes).map(code => {
          return <AutoComplete.Option value={code}>
...
function K8SClusterSetting(props) {
  ...
  return <SettingBlock title={"K8S集群设置"}>
    <Table
      size={"small"}
      pagination={false}
      columns={[
        ...
        {
          title: 'Env', dataIndex: 'env', render: (value = []) => value.map(env => <Tag>{env}</Tag>)
        },
        {
          title: '操作',
          render: (_, record, index) => {
            return <>
              ...
              <Button shape={"circle"} onClick={() => onEdit(index)} style={{marginLeft: '10px'}}>
                <EditFilled/>
              </Button>
            </>
          }
