...
import {createCollection, createRequest, deleteCollection, deleteTestCase, getFolderTree} from "@/services/httptest";
            ...
            <ReactScrollBar style={{height: '710px'}}>
              <Tree.DirectoryTree
                ...
                {(collections || []).map(item => {
                  return <Tree.TreeNode key={`collection-${item.id}`} collectionID={item.id} title={item.name}>
                    {(item.test_cases || []).map(testCase => {
                      return <Tree.TreeNode key={`testcase-${testCase.id}`} collectionID={item.id} title={testCase.name}
                                            isLeaf/>
                    })}
                  </Tree.TreeNode>
                })}
