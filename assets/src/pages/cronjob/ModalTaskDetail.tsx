...
export default function ModalTaskDetail(props: ModalTaskDetailProps) {
  ...
  return (
    <Modal {...props} title={'任务详情'} footer={null} width={'900px'}>
        ...
        <div>
          <Descriptions bordered column={4}>
            ...
            <Descriptions.Item label={'Status'} span={4}>
              <Badge
                status={convertTaskStatusToBadgeStatus(taskDeTail?.status)}
                text={taskDeTail?.status}
              />
            </Descriptions.Item>

            <Descriptions.Item label={'Executed At'} span={2}>
              {taskDeTail?.executed_at}
            </Descriptions.Item>
            <Descriptions.Item label={'Finished At'} span={2}>
