...
function JobFormFields(props: JobFormProps) {
  ...
  useEffect(() => {
    appName &&
      ServiceAppEnvZone(appName).then((r) => {
  ...
  useEffect(() => {
    if (appName && env && zone) {
      ServiceAppNodeList({ app_name: appName, env: env, zone_code: zone, pageSize: 10000 }).then(
        (r) => {
    ...
    <>
      ...
      <Row gutter={10}>
        <Col span={8}>
          ...
          >
            <Select showSearch onChange={onAppChange}>
              {appList.map((item) => (
                <Select.Option key={item.app_name} value={item.app_name}>
                  {item.app_name}
                </Select.Option>
              ))}
            </Select>
        ...
        <Col span={8}>
          <Form.Item label={'Env'} name={'env'} rules={[{ required: true, message: '请选择环境' }]}>
            <Select
              onSelect={(val: string) => {
                setEnv(val);
        ...
        <Col span={8}>
          <Form.Item
            label={'Zone'}
            name={'zone'}
            rules={[{ required: true, message: '请选择Zone' }]}
          >
            <Select onSelect={(zone: string) => setZone(zone)}>
              {zones &&
                zones?.map((zone: { zone_code: React.ReactText; zone_name: React.ReactNode }) => (
                  <Select.Option key={zone.zone_code} value={zone.zone_code}>
                    {zone.zone_name}
                  </Select.Option>
                ))}
            </Select>
      ...
      <Row gutter={10}>
        ...
        <Col span={6}>
          <Form.Item label={'Timeout (s)'} name={'timeout'} initialValue={3}>
            <InputNumber width={'100%'} min={3} max={3600} />
          </Form.Item>
        </Col>

        <Col span={6}>
          <Form.Item label={'Retries'} initialValue={0} name={'retry_count'}>
            <InputNumber width={'100%'} min={0} max={10} />
          </Form.Item>
        </Col>

        <Col span={6}>
          <Form.Item initialValue={3} label={'Retry Interval (s)'} name={'retry_interval'}>
            <InputNumber width={'100%'} min={0} max={30} />
          </Form.Item>
        </Col>
      </Row>
          ...
          return (
            <div>
              {fields.map((field, index) => (
                <Form.Item key={field.key} label={`Timer ${index}`}>
                  <Form.Item
                  ...
                  <Form.Item noStyle>
                    <Button onClick={() => operation.remove(index)} size={'small'}>
                      remove timer
                    </Button>
                  </Form.Item>
                </Form.Item>
