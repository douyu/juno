...
function getColumns(options: { apps: AppItem[] }) {
  const columns: ProColumns<Job>[] = [
    ...
    },
    {
      ...
      render(val: any) {
        return (
          <>
            {(val as Timer[]).map((timer, idx) => (
              <Tag key={idx} icon={<ClockCircleOutlined />} color={'processing'}>
                {timer.cron}
              </Tag>
            ))}
          </>
        ...
        return (
          <Select showSearch allowClear>
            {options.apps.map((app) => (
              <Select.Option key={app.app_name} value={app.app_name}>
                {app.app_name}
              </Select.Option>
            ))}
          </Select>
            ...
            render: (val, row, _) => {
              return (
                <Button.Group>
                  ...
                  <Button type={'link'} onClick={() => onTrigger(row)}>
                    手动触发
                  </Button>
                </Button.Group>
              );
