    ...
    <div className={styles.Test}>
      ...
      {!pipeline && (
        <div style={{ flex: '1 1 auto' }}>
          ...
          >
            <h4>Pipeline</h4>
            <Button icon={<FileAddOutlined />} onClick={() => visibleModalCreateAction.setTrue()}>
              新建 Pipeline
            </Button>
          </div>

          {!pipelinesLoading && (
            <ul className={styles.Pipelines}>
              {pipelines.map((item) => (
                <li
                  key={item.id}
                  onClick={() => {
          ...
          {!pipelinesLoading && (!pipelines || !pipelines.length) && (
            <Empty description={'暂无Pipeline'} />
          )}
        </div>
      )}
