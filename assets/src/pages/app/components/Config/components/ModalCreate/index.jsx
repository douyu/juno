...
function ModalCreate(props) {
  ...
  return (
    <Modal {...props} title={'新增配置'} onOk={onOK}>
      <Form form={form} labelCol={{ span: 4 }} onFinish={onFinish}>
        <Form.Item label={'Zone'} name={'zone'} rules={[{ required: true }]}>
          <Select>
            {props.zoneList.map((item, index) => {
              return (
                <Select.Option value={item.zone_code} key={index}>
                  {item.zone_name}
                </Select.Option>
              );
            })}
        ...
        >
          ...
          >
            <Radio.Button key={'toml'} value={'toml'}>
              toml
            </Radio.Button>
            <Radio.Button key={'yaml'} value={'yaml'}>
              yaml
            </Radio.Button>
            <Radio.Button key={'ini'} value={'ini'}>
              ini
            </Radio.Button>
          </Radio.Group>
        </Form.Item>
        <Form.Item
          ...
          rules={[
            ...
            { min: 1, max: 32, message: '文件名长度在1到32之间' },
            {
              pattern: new RegExp(/^[a-zA-Z][a-zA-Z0-9_-]+$/),
              message: '文件名只能包含字母数字 _ 和 -，并且只能以字母开头',
            },
          ]}
        >
          <Input placeholder={'不带后缀的文件名，示例: config-prod'} suffix={'.' + format} />
        </Form.Item>
      </Form>
    </Modal>
  );
